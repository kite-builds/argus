...
public struct ResearchSession<phantom T> has key {
    ...
    paid_to: vector<address>,
    /// 32-byte Walrus blob hashes for each paid response, parallel to
    /// `paid_to`. Hash commits to the blob the agent observed; does
    /// NOT prove the endpoint produced that blob.
    response_blob_hashes: vector<vector<u8>>,
...
public struct ReceiptValue has copy, drop, store {
    amount: u64,
    blob_hash: vector<u8>,
    /// Tx index inside the session — matches position in `paid_to`.
    sequence: u64,
}
...
/// @notice Emitted when a new research session opens.
...
/// @notice Emitted on each (payment + blob commitment) inside a PTB.
/// @dev `sequence` is the index inside the session, useful for
...
/// @notice Emitted when the session is finalised by the owner.
...
/// @notice Emitted when an extra reader is added to the Seal allowlist.
...
/// @notice Open a research session pre-funded with a hard budget cap.
/// @dev Session is shared so PTBs from the agent SDK can refer to it
///      by id. `min_sources = 0` disables the multi-source floor.
public fun mint_session<T>(
    config: &ArgusConfig,
    question_blob_id: String,
    budget: Coin<T>,
    min_sources: u64,
...
/// @notice Pay an endpoint AND commit its response-blob hash atomically.
/// @dev The blob is already stored in Walrus before this is called.
...
public fun pay_and_record<T>(
    ...
    let sequence = vector::length(&session.paid_to);
    session.total_paid = session.total_paid + amount;
    vector::push_back(&mut session.paid_to, payee);
    vector::push_back(&mut session.response_blob_hashes, blob_hash);

    df::add(&mut session.id, key, ReceiptValue {
        amount,
        blob_hash,
        sequence,
    });
...
/// @notice Owner-only: write the final synthesised answer blob and
///         emit the locked event. After this, no more receipts can be
///         recorded.
/// @dev Enforces `min_sources` so a thin single-source answer can be
...
public fun lock_session<T>(
    ...
    assert!(!session.locked, EAlreadyLocked);
    assert!(
        vector::length(&session.paid_to) >= session.min_sources,
        EMinSourcesNotMet,
    );
    ...
    event::emit(SessionLocked {
        ...
        response_blob_id,
        total_paid: session.total_paid,
        num_sources: vector::length(&session.paid_to),
    });
}
...
/// @notice Owner-only: extend Seal decryption rights to another address.
...
/// @notice Owner-only: reclaim any unspent budget after the session is
///         locked.
/// @dev Locking is required so the budget can't be withdrawn out from
...
/// @notice Seal-protocol decryption gate. Seal calls this with the
...
public fun paid_to<T>(s: &ResearchSession<T>): &vector<address> { &s.paid_to }
public fun response_blob_hashes<T>(s: &ResearchSession<T>): &vector<vector<u8>> {
    &s.response_blob_hashes
}
...
/// @notice Look up a stored receipt by (payee, nonce). Returns the
...
public fun receipt<T>(
    ...
    let key = ReceiptKey<T> { payee, nonce };
    let v: &ReceiptValue = df::borrow(&session.id, key);
